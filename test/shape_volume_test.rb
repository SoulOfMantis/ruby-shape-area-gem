...
class TestSolidShapesCombined < Minitest::Test
  ...
  def setup
    ...
    @trapezoid = Trapezoid.new(3, 5, 4, 4, 2)
  end
  ...
  def test_right_cuboid
    rc = Right_cuboid.new(2, 3, 4)
    assert_equal 24, rc.volume
    assert_equal 0, Right_cuboid.new(0, 5, 10).volume
    assert_equal 0, Right_cuboid.new(5, 0, 10).volume
    assert_equal 0, Right_cuboid.new(5, 10, 0).volume
    assert_raises(ArgumentError) { Right_cuboid.new(-2, -3, 10) }
    assert_raises(ArgumentError) { Right_cuboid.new(-2, 0, 10) }
    float_cuboid = Right_cuboid.new(2.5, 3.5, 4.5)
  ...
  def test_cone
    circle = Circle.new(3)
    cone = Cone.new(circle, 5)
    expected_volume = 47.12388980384689
    assert_in_delta expected_volume, cone.volume, DELTA
    assert_equal 0, Cone.new(circle, 0).volume
    assert_raises(ArgumentError) { Cone.new(circle, -5) }
    zero_circle = Circle.new(0)
    assert_equal 0, Cone.new(zero_circle, 5).volume
    assert_raises(ArgumentError) { Cone.new(10, 5) }
    tiny_circle = Circle.new(0.001)
    tiny_cone = Cone.new(tiny_circle, 10)
  ...
  def test_hexagon_prism
    hp = Regular_hexagonal_prism.new(2, 5)
    hex_area = 10.392304845413264
    expected_volume = hex_area * 5
    assert_in_delta expected_volume, hp.volume, DELTA
    assert_equal 0, Regular_hexagonal_prism.new(0, 5).volume
    assert_equal 0, Regular_hexagonal_prism.new(2, 0).volume
    assert_raises(ArgumentError) { Regular_hexagonal_prism.new(-2, 5) }
  end
